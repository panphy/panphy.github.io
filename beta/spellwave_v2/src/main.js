...
const REST_WAVE_INTERVAL = 4;
const REST_HEAL_INTERVAL = 0.32;
...
startButton.addEventListener('click', () => {
  ...
  if (mode === 'paused') {
    ...
    advanceWaveSet();
  } else if (mode === 'resting' && health >= MAX_LIFE) {
    advanceWaveSet();
  } else {
    startGame();
  }
...
function startGame() {
  ...
  inputTrace = '';
  document.body.classList.remove('is-god-mode', 'is-resting');
  if (godModeBadge) { godModeBadge.remove(); godModeBadge = null; }
  setRestSceneActive(false);
  score = 0;
  health = MAX_LIFE;
  renderedHealth = null;
  waveSet = 1;
  ...
  messagePanel.classList.remove('is-cleared', 'is-resting-message');
...
function showMessage(kicker, title, scoreText, buttonText, copyText) {
  ...
  messagePanel.classList.remove('is-resting-message');
...
function handleKeyDown(event) {
  ...
  if (event.key === 'Enter' && mode === 'resting' && health >= MAX_LIFE) {
    event.preventDefault();
    advanceWaveSet();
    return;
  }
...
function updateEnvironment(seconds, delta) {
  if (mode === 'resting') {
    camera.position.x = 1.05 + Math.sin(seconds * 0.34) * 0.055;
    camera.position.y = 3.0 + Math.sin(seconds * 0.45) * 0.035;
    camera.position.z = -4.0;
    camera.lookAt(reusableVector.set(0.9, 1.55, -8.65));
    if (moon) moon.visible = false;
    if (moonHaze) moonHaze.style.opacity = '0';
    updateRestScene(seconds, delta);
    return;
  }
...
function updatePhaseDisplay() {
  ...
  if (mode === 'resting') {
    phaseValue.textContent = `Resting · Wave ${waveSet} cleared`;
    return;
  }
  if (mode === 'gameover') {
...
function ellipsoidMesh(width, height, depth, material, x, y, z, segments = 18) {
  const mesh = new THREE.Mesh(new THREE.SphereGeometry(0.5, segments, Math.max(10, Math.floor(segments * 0.58))), material);
...
function cylinderMesh(radiusTop, radiusBottom, height, material, x, y, z, segments = 18) {
...
function coneMesh(radius, height, material, x, y, z, segments = 18) {
...
function createRestScene() {
  ...
  const makeBasic = (color, opacity = 1) => new THREE.MeshBasicMaterial({
    color,
  ...
  const floorMat = makeMat(0x3b261b);
  const plankMat = makeMat(0x5a3822);
  const wallMat = makeMat(0x2b1d22);
  const beamMat = makeMat(0x1b1110);
  const stoneMat = makeMat(0x4c4c52);
  const stoneDarkMat = makeMat(0x25262c);
  const snowMat = makeMat(0xddeeff, { emissive: 0x8fc5ff, emissiveIntensity: 0.08 });
  const windowMat = makeBasic(0x172c4d, 0.92);
  const rugMat = makeMat(0x7f2440);
  const rugTrimMat = makeMat(0xefc35c);
  const chairMat = makeMat(0x7c3b24);
  const cushionMat = makeMat(0x124f5a);
  const robeMat = makeMat(0x334a8a);
  const robeTrimMat = makeMat(0x58dfcf, { emissive: 0x0e6861, emissiveIntensity: 0.22 });
  const faceMat = makeMat(0xd9aa74);
  const beardMat = makeMat(0xf2f0df);
  const hatMat = makeMat(0x24305f);
  const browMat = makeMat(0xd7d2c4);
  const goldMat = makeMat(0xefc35c, { emissive: 0x6f4c12, emissiveIntensity: 0.12 });
  const cheekMat = makeBasic(0xff9fa8, 0.68);
  const eyeMat = makeMat(0x12121a);
  const eyeGlintMat = makeBasic(0xffffff, 0.72);
  const beltMat = makeMat(0x1e2c70);
  const mouthMat = makeMat(0x8a4a3a);
  const bookMats = [makeMat(0xb44742), makeMat(0x2d7f5e), makeMat(0xd6a53f), makeMat(0x4d5da8)];
  const fireMats = [makeBasic(0xfff1a3, 0.95), makeBasic(0xff9c35, 0.82), makeBasic(0xe74727, 0.72)];
  const teaSteamMat = makeBasic(0xe9fbff, 0.34);

  group.add(blockMesh(16, 0.34, 12, floorMat, 0, -0.28, -9.3));
  for (let x = -7.2; x <= 7.2; x += 1.2) group.add(blockMesh(0.06, 0.04, 11.6, plankMat, x, -0.08, -9.3));
  group.add(blockMesh(16.4, 6.2, 0.42, wallMat, 0, 2.8, -15.25));
  group.add(blockMesh(0.42, 5.5, 11.4, wallMat, -8.1, 2.45, -9.8));
  group.add(blockMesh(0.42, 5.5, 11.4, wallMat, 8.1, 2.45, -9.8));
  for (const x of [-6.6, -3.3, 0, 3.3, 6.6]) group.add(blockMesh(0.32, 5.9, 0.52, beamMat, x, 2.7, -15.0));
  group.add(blockMesh(16.2, 0.32, 0.54, beamMat, 0, 5.32, -15.0));

  group.add(blockMesh(3.4, 2.35, 0.18, windowMat, 3.8, 3.25, -15.0));
  group.add(blockMesh(3.7, 0.18, 0.3, beamMat, 3.8, 4.48, -14.82));
  group.add(blockMesh(3.7, 0.18, 0.3, beamMat, 3.8, 2.02, -14.82));
  group.add(blockMesh(0.18, 2.6, 0.3, beamMat, 1.95, 3.25, -14.82));
  group.add(blockMesh(0.18, 2.6, 0.3, beamMat, 5.65, 3.25, -14.82));
  group.add(blockMesh(0.14, 2.3, 0.28, beamMat, 3.8, 3.25, -14.8));
  group.add(blockMesh(3.25, 0.14, 0.28, beamMat, 3.8, 3.25, -14.78));
  group.add(blockMesh(1.3, 0.2, 0.12, snowMat, 2.9, 2.23, -14.62));
  group.add(blockMesh(1.4, 0.22, 0.12, snowMat, 4.75, 4.18, -14.62));

  group.add(blockMesh(4.8, 0.08, 3.0, rugMat, 0.8, -0.04, -8.3));
  group.add(blockMesh(5.15, 0.09, 0.18, rugTrimMat, 0.8, 0.01, -6.78));
  group.add(blockMesh(5.15, 0.09, 0.18, rugTrimMat, 0.8, 0.01, -9.82));
  group.add(blockMesh(0.18, 0.09, 3.1, rugTrimMat, -1.82, 0.01, -8.3));
  group.add(blockMesh(0.18, 0.09, 3.1, rugTrimMat, 3.42, 0.01, -8.3));

  group.add(blockMesh(2.9, 0.62, 1.0, stoneMat, -3.85, 0.28, -14.1));
  group.add(blockMesh(3.35, 2.5, 0.54, stoneMat, -3.85, 1.62, -14.42));
  group.add(blockMesh(2.0, 1.55, 0.62, stoneDarkMat, -3.85, 1.08, -14.08));
  group.add(blockMesh(3.75, 0.34, 0.8, stoneDarkMat, -3.85, 2.85, -14.22));
  group.add(blockMesh(1.2, 3.0, 0.72, stoneMat, -3.85, 4.28, -14.35));
  group.add(blockMesh(1.35, 0.18, 0.34, makeMat(0x6b341a), -4.18, 0.6, -13.72));
  group.add(blockMesh(1.25, 0.18, 0.34, makeMat(0x7f3f1f), -3.42, 0.72, -13.68));
  const flames = [
    blockMesh(0.34, 0.72, 0.24, fireMats[2], -4.18, 1.0, -13.64),
    blockMesh(0.3, 0.96, 0.22, fireMats[1], -3.84, 1.16, -13.58),
    blockMesh(0.24, 0.62, 0.2, fireMats[0], -3.52, 1.04, -13.62),
  ];
  for (const flame of flames) group.add(flame);

  const chair = new THREE.Group();
  chair.position.set(1.95, 0, -8.7);
  chair.rotation.y = -0.16;
  chair.add(blockMesh(1.8, 0.46, 1.42, cushionMat, 0, 0.46, 0));
  chair.add(blockMesh(1.95, 2.05, 0.42, chairMat, 0, 1.42, -0.52));
  chair.add(blockMesh(0.34, 1.15, 1.36, chairMat, -1.1, 0.92, 0.08));
  chair.add(blockMesh(0.34, 1.15, 1.36, chairMat, 1.1, 0.92, 0.08));
  group.add(chair);

  const wizard = new THREE.Group();
  wizard.position.set(1.9, 0.62, -8.35);
  wizard.rotation.y = -0.15;
  const legMat = makeMat(0x2a3f7a);
  wizard.add(blockMesh(0.88, 1.0, 0.6, robeMat, 0, 0.82, 0));
  wizard.add(blockMesh(0.95, 0.15, 0.66, robeTrimMat, 0, 0.34, 0.02));
  wizard.add(blockMesh(0.9, 0.1, 0.62, beltMat, 0, 0.82, 0.01));
  wizard.add(blockMesh(0.08, 0.13, 0.07, goldMat, 0, 0.82, 0.36));
  wizard.add(blockMesh(0.052, 0.6, 0.055, robeTrimMat, 0, 0.72, 0.32));
  wizard.add(blockMesh(0.12, 0.12, 0.08, goldMat, -0.22, 0.76, 0.33));
  wizard.add(blockMesh(0.12, 0.12, 0.08, goldMat, 0.22, 0.64, 0.33));
  wizard.add(blockMesh(0.22, 0.18, 0.7, robeMat, -0.54, 0.96, 0.1));
  wizard.add(blockMesh(0.22, 0.18, 0.6, robeMat, 0.54, 0.96, 0.14));
  wizard.add(blockMesh(0.25, 0.09, 0.11, robeTrimMat, -0.54, 0.96, 0.48));
  wizard.add(blockMesh(0.25, 0.09, 0.11, robeTrimMat, 0.54, 0.96, 0.45));
  wizard.add(ellipsoidMesh(0.13, 0.1, 0.12, faceMat, -0.54, 0.88, 0.55, 10));
  wizard.add(ellipsoidMesh(0.13, 0.1, 0.12, faceMat, 0.54, 0.88, 0.52, 10));
  wizard.add(blockMesh(0.5, 0.26, 0.54, legMat, -0.34, 0.18, 0.1));
  wizard.add(blockMesh(0.5, 0.26, 0.54, legMat, 0.34, 0.18, 0.1));
  wizard.add(ellipsoidMesh(0.58, 0.54, 0.52, faceMat, 0, 1.55, 0.03, 20));
  wizard.add(ellipsoidMesh(0.64, 0.22, 0.2, beardMat, 0, 1.35, 0.3, 18));
  wizard.add(ellipsoidMesh(0.48, 0.25, 0.17, beardMat, 0, 1.11, 0.25, 16));
  wizard.add(ellipsoidMesh(0.24, 0.26, 0.14, beardMat, 0, 0.89, 0.19, 12));
  wizard.add(blockMesh(0.24, 0.055, 0.07, beardMat, -0.09, 1.45, 0.37));
  wizard.add(blockMesh(0.24, 0.055, 0.07, beardMat, 0.09, 1.45, 0.37));
  wizard.add(cylinderMesh(0.43, 0.45, 0.18, hatMat, 0, 1.88, 0.02, 20));
  wizard.add(coneMesh(0.34, 0.84, hatMat, 0, 2.32, 0.02, 20));
  wizard.add(cylinderMesh(0.445, 0.445, 0.056, robeTrimMat, 0, 1.95, 0.02, 20));
  wizard.add(blockMesh(0.1, 0.1, 0.08, goldMat, -0.15, 2.25, 0.27));
  wizard.add(blockMesh(0.08, 0.08, 0.08, goldMat, 0.17, 2.45, 0.2));
  wizard.add(blockMesh(0.13, 0.13, 0.08, eyeMat, -0.16, 1.62, 0.28));
  wizard.add(blockMesh(0.13, 0.13, 0.08, eyeMat, 0.16, 1.62, 0.28));
  wizard.add(ellipsoidMesh(0.04, 0.04, 0.03, eyeGlintMat, -0.13, 1.65, 0.33, 6));
  wizard.add(ellipsoidMesh(0.04, 0.04, 0.03, eyeGlintMat, 0.19, 1.65, 0.33, 6));
  wizard.add(blockMesh(0.09, 0.055, 0.07, browMat, -0.11, 1.74, 0.3));
  wizard.add(blockMesh(0.09, 0.055, 0.07, browMat, -0.21, 1.735, 0.3));
  wizard.add(blockMesh(0.09, 0.055, 0.07, browMat, 0.11, 1.74, 0.3));
  wizard.add(blockMesh(0.09, 0.055, 0.07, browMat, 0.21, 1.735, 0.3));
  wizard.add(ellipsoidMesh(0.1, 0.12, 0.11, faceMat, 0, 1.52, 0.37, 12));
  wizard.add(ellipsoidMesh(0.095, 0.055, 0.04, cheekMat, -0.27, 1.48, 0.36, 10));
  wizard.add(ellipsoidMesh(0.095, 0.055, 0.04, cheekMat, 0.27, 1.48, 0.36, 10));
  wizard.add(blockMesh(0.08, 0.04, 0.06, mouthMat, -0.07, 1.41, 0.38));
  wizard.add(blockMesh(0.08, 0.04, 0.06, mouthMat, 0.07, 1.41, 0.38));
  group.add(wizard);

  group.add(blockMesh(1.22, 0.22, 0.72, makeMat(0x5e1f45), 1.9, 1.18, -8.0));
  group.add(blockMesh(1.1, 0.09, 0.68, makeMat(0xefc35c), 1.9, 1.33, -7.96));
  group.add(blockMesh(0.42, 0.18, 0.58, makeMat(0x2a1410), 1.18, 0.13, -6.05));
  group.add(blockMesh(0.42, 0.18, 0.58, makeMat(0x2a1410), 1.82, 0.13, -5.94));
  group.add(blockMesh(2.1, 0.2, 0.82, makeMat(0x5b351e), -0.42, 0.48, -5.78));
  group.add(blockMesh(0.18, 0.52, 0.18, makeMat(0x3b2418), -1.32, 0.22, -5.44));
  group.add(blockMesh(0.18, 0.52, 0.18, makeMat(0x3b2418), 0.48, 0.22, -5.44));
  group.add(blockMesh(0.72, 0.08, 0.48, makeMat(0xf2f0df), -0.78, 0.64, -5.58));
  group.add(blockMesh(0.72, 0.08, 0.48, makeMat(0xf2f0df), -0.08, 0.64, -5.58));
  group.add(blockMesh(0.08, 0.1, 0.54, makeMat(0xefc35c), -0.44, 0.68, -5.55));
  group.add(cylinderMesh(0.09, 0.11, 0.32, makeBasic(0xffd36e, 0.86), 0.46, 0.82, -5.82, 16));
  group.add(cylinderMesh(0.14, 0.14, 0.08, makeMat(0xf2f0df), 0.46, 0.62, -5.82, 16));
  group.add(blockMesh(0.82, 0.07, 0.66, makeMat(0x5e3a22), 2.75, 0.895, -7.83));
  group.add(cylinderMesh(0.055, 0.055, 0.86, makeMat(0x3b2418), 2.75, 0.43, -7.83, 8));
  group.add(cylinderMesh(0.22, 0.22, 0.02, makeMat(0xf2f0df), 2.75, 0.93, -7.85, 18));
  const mug = cylinderMesh(0.18, 0.16, 0.34, makeMat(0xf2f0df), 2.75, 1.1, -7.85, 18);
  group.add(mug);
  group.add(cylinderMesh(0.14, 0.14, 0.04, makeBasic(0xffb36a, 0.92), 2.75, 1.29, -7.85, 18));
  group.add(cylinderMesh(0.1, 0.08, 0.24, makeBasic(0xfff1a3, 0.9), -4.92, 2.15, -13.72, 16));
  group.add(cylinderMesh(0.1, 0.08, 0.24, makeBasic(0xffd36e, 0.82), -2.75, 2.2, -13.72, 16));
  group.add(blockMesh(0.12, 1.25, 0.12, makeMat(0x2a1a10), -2.62, 1.46, -13.58));
  group.add(blockMesh(0.42, 0.1, 0.12, makeMat(0x2a1a10), -2.78, 2.04, -13.58));

  group.add(blockMesh(1.9, 0.18, 0.5, beamMat, -6.0, 3.1, -14.8));
  for (let index = 0; index < 10; index += 1) {
    group.add(blockMesh(0.16, 0.58 + (index % 3) * 0.08, 0.42, bookMats[index % bookMats.length], -6.78 + index * 0.18, 3.5, -14.6));
  }

  group.add(blockMesh(1.28, 0.32, 1.0, stoneMat, 5.8, 0.26, -8.55));
  group.add(blockMesh(1.05, 0.16, 0.8, makeBasic(0x58dfcf, 0.68), 5.8, 0.56, -8.55));
  group.add(blockMesh(0.28, 0.48, 0.28, makeMat(0x6b341a), 5.16, 0.58, -8.55));
  group.add(blockMesh(0.28, 0.48, 0.28, makeMat(0x6b341a), 6.44, 0.58, -8.55));

  const steam = [];
  const steamSeeds = [[2.73, 1.38, -7.82], [2.77, 1.46, -7.88]];
  for (let index = 0; index < steamSeeds.length; index += 1) {
    const material = teaSteamMat.clone();
    const [x, y, z] = steamSeeds[index];
    const puff = ellipsoidMesh(0.055, 0.22, 0.055, material, x, y, z, 10);
    puff.castShadow = false;
    puff.receiveShadow = false;
    group.add(puff);
    steam.push({ mesh: puff, material, baseX: x, baseY: y, phase: index * 1.7 });
  }

  const fireLight = new THREE.PointLight(0xff9c35, 5.0, 17, 1.55);
  fireLight.position.set(-3.8, 1.7, -12.4);
  group.add(fireLight);
  const windowLight = new THREE.PointLight(0x7dd3fc, 1.25, 12, 2);
  windowLight.position.set(3.8, 3.2, -13.0);
  group.add(windowLight);

  restScene = { group, flames, steam, wizard, mug, fireLight };
}

function setRestSceneActive(active) {
  if (restScene) restScene.group.visible = active;
  world.visible = !active;
  enemyGroup.visible = !active;
  effectsGroup.visible = !active;
  labelsLayer.hidden = active;
  for (const cloud of clouds) cloud.group.visible = active ? false : !cloud.weatherOnly;
  if (active) {
    scene.background.setHex(0x17111f);
    scene.fog.color.setHex(0x17111f);
    scene.fog.near = 16;
    scene.fog.far = 54;
    hemiLight.color.setHex(0xffd6a8);
    hemiLight.groundColor.setHex(0x1b1110);
    hemiLight.intensity = 0.9;
    moonLight.visible = false;
    emberLight.intensity = 0;
  } else {
    labelsLayer.hidden = false;
    moonLight.visible = true;
  }
}

function updateRestScene(seconds) {
  if (!restScene) return;
  restScene.fireLight.intensity = 4.5 + Math.sin(seconds * 13.0) * 0.55 + Math.sin(seconds * 19.0) * 0.24;
  for (let index = 0; index < restScene.flames.length; index += 1) {
    const flame = restScene.flames[index];
    const pulse = 1 + Math.sin(seconds * (8.2 + index * 1.3) + index) * 0.16;
    flame.scale.set(0.9 + pulse * 0.1, pulse, 0.92 + pulse * 0.08);
  }
  restScene.wizard.position.y = 0.62 + Math.sin(seconds * 1.8) * 0.025;
  restScene.mug.position.y = 1.1 + Math.sin(seconds * 2.2) * 0.012;
  for (const puff of restScene.steam) {
    const rise = (seconds * 0.22 + puff.phase) % 1;
    puff.mesh.position.y = puff.baseY + rise * 0.9;
    puff.mesh.position.x = puff.baseX + Math.sin(seconds * 1.6 + puff.phase) * 0.055;
    puff.mesh.scale.setScalar(0.72 + rise * 0.92);
    puff.material.opacity = (1 - rise) * 0.36;
  }
}
...
function shouldStartRestBreak() {
  return waveSet > 0 && waveSet % REST_WAVE_INTERVAL === 0;
}
...
function startRestBreak() {
  ...
  setRestSceneActive(true);
  showMessage(
    `WINTER ${waveSet} CLEARED`,
    'Rest Stop',
    `Score ${formatScore(score)}`,
    health >= MAX_LIFE ? 'Continue' : 'Restoring...',
    'The wizard warms up by the hearth while the spell hearts refill.'
  );
  messagePanel.classList.add('is-resting-message');
  setRestContinueReady(health >= MAX_LIFE);
  updateTypedDisplay();
  updateHud(true);
  updatePhaseDisplay();
}

function updateRestBreak(delta) {
  if (health >= MAX_LIFE) return;
  restHealTimer += delta;
  let healed = false;
  while (restHealTimer >= REST_HEAL_INTERVAL && health < MAX_LIFE) {
    restHealTimer -= REST_HEAL_INTERVAL;
    health += 1;
    healed = true;
    playHealSound();
    updateHud(true);
  }
  if (health >= MAX_LIFE) setRestContinueReady(true);
  else if (healed) setRestContinueReady(false);
}

function setRestContinueReady(isReady) {
  startButton.disabled = !isReady;
  startButton.textContent = isReady ? 'Continue' : 'Restoring...';
  messageScore.textContent = isReady
    ? `Life full · Score ${formatScore(score)}`
    : `Restoring life · ${formatHeartCount(health)}/${HEART_COUNT} hearts`;
  messageCopy.textContent = isReady
    ? 'Hearts are full. Continue into the next cycle when ready.'
    : 'The wizard warms up by the hearth while the spell hearts refill.';
}
